...
- Trigger rules: If the user names a skill (with `$SkillName` or plain text) OR the task clearly matches a skill's description shown above, you must use that skill for that turn. Treat requests such as "깃허브에 올려줘" or "GitHub에 올려줘" as matching `git-collaboration-flow` with the default expectation of branch push, PR creation or preparation, and main-branch reflection when repository permissions and workflow allow it; once main-branch reflection completes, follow with `vercel-deploy` by default as a preview deploy unless the user explicitly asks for a different deployment target or to skip deploy. Treat requests such as "브랜치 파서 PR까지 해줘", "PR 준비해줘", "main에 반영되게 진행해줘", or "push 해줘" as matching `git-collaboration-flow` as well unless the user explicitly asks for a different workflow. Treat requests about deploying after merge, pushing live after main updates, or automatic deploy after main as matching `vercel-deploy`. Multiple mentions mean use them all. Do not carry skills across turns unless re-mentioned.
